...
def test_split_nodes_delimiter_bold():
    old_node = [TextNode("This is text with a **bolded phrase** in the middle", TextType.MD_TEXT)]
    delimiter = "**"
    expected = [
...
def test_split_nodes_delimiter_multiple_blocks():
    old_node = [TextNode("This is text with a `code block`, more text with `code`", TextType.MD_TEXT)]
    delimiter = "`"
    expected = [
        TextNode("This is text with a ", TextType.MD_TEXT),
        TextNode("code block", TextType.MD_CODE),
...
def test_split_nodes_delimiter_multiple_nodes():
    old_node = [
            TextNode("This is text with a `code block`, more text with `code`", TextType.MD_TEXT),
...
def test_split_nodes_delimiter_non_text_node():
    old_node = [TextNode("This is text with a `code block`, more text with `code`", TextType.MD_CODE)]
    delimiter = "`"
    expected = [TextNode("This is text with a `code block`, more text with `code`", TextType.MD_CODE)]
